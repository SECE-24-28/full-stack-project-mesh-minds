...
const BAR_COLORS = ['#9FA1FF', '#6BB6FF', '#7EDC92', '#F59E0B', '#F87171', '#06B6D4'];
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <div className="saas-card p-6">
        ...
        <p className="section-subtitle mb-5">How proposals spread across event types</p>
            ...
            <div className="flex-1 space-y-2 min-w-0">
              {categoryData.map((c) => (
                <div key={c.name} className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-xs font-bold text-[#1E293B] flex-shrink-0">{c.value}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="saas-card p-6">
        ...
        <p className="section-subtitle mb-5">Total contributions grouped by category</p>
        {fundingChartData.length === 0 ? (
          <EmptyChart message="No funding contributions yet." />
        ) : (
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={fundingChartData} barSize={24}>
      ...
      <div className="saas-card p-6 lg:col-span-2">
        ...
        <p className="section-subtitle mb-5">Current pipeline breakdown across all stages</p>
        {statusData.length === 0 ? (
          <EmptyChart />
        ) : (
              ...
              {statusData.map((s) => (
                <div key={s.name} className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-[#0F172A] leading-tight">{s.value}</p>
                  </div>
                </div>
              ))}
            </div>
