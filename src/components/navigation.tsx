...
import {
  ...
  Zap, Users, Shield, GraduationCap, BookOpen,
  Star, User, CalendarDays, Clock, ChevronRight, CheckCircle, Settings,
} from 'lucide-react';
...
const navByRole = {
  ...
  ADMIN: [
    ...
    { href: '/admin/pending-events',   label: 'Event Management', icon: Clock },
    { href: '/admin/completed-events', label: 'Completed Events', icon: CheckCircle },
    { href: '/analytics',   label: 'Analytics',         icon: BarChart3 },
...
export function Sidebar() {
  ...
  return (
    <aside className="app-sidebar fixed left-0 top-0 z-30 flex h-screen flex-col" data-role={role}>
      {/* Logo */}
      <div className="sidebar-logo-area flex items-center gap-3 px-6">
        <div className="flex h-9 w-9 items-center justify-center rounded-xl shadow-sm" style={{ background: 'var(--role-accent)' }}>
      ...
      {session?.user && (
        <div className="mx-4 mt-5 mb-2 rounded-2xl p-3.5" style={{ background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.15)' }}>
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl text-white shadow-sm flex-shrink-0" style={{ background: 'var(--role-accent)' }}>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={cn('sidebar-nav-item', isActive && 'active')}>
              ...
              {isActive && <ChevronRight className="h-3.5 w-3.5 flex-shrink-0" style={{ color: 'var(--role-primary)' }} />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="px-3 py-4 space-y-0.5" style={{ borderTop: '1px solid rgba(255,255,255,0.12)' }}>
        ...
        <button onClick={handleLogout} className="sidebar-nav-item w-full text-left hover:!bg-red-500/20 hover:!text-red-200">
